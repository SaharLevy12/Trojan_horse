...
def send_decrypter_to_client(conn):
    send_file_size(conn)
    
    with open("decrypter.exe", 'rb') as file:
        chunk = file.read(4096)
        while chunk:
            conn.sendall(chunk)
            chunk = file.read(4096)

def send_file_size(conn):
    size = os.path.getsize("decrypter.exe")
...
def main():
    ...
    send_decrypter_to_client(secured_conn)
